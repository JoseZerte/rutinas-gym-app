...
// --- ESTILOS ORIGINALES (Mantenidos tal cual) ---
const GlobalStyles = () => (
    <style>{`
    * {
        ...
        <div ref={setNodeRef} style={style} className="bg-[#161b22] border border-gray-800 rounded-2xl mb-8 overflow-hidden shadow-xl">
            ...
            <div className="p-4 bg-[#161b22]">
                ...
                <button
                    ...
                    className="w-full py-4 mt-2 border border-dashed border-gray-700 bg-gray-800/20 rounded-xl text-gray-500 font-bold text-sm tracking-wide active:bg-blue-500/5 active:text-blue-400 transition-all"
                >
                    + AÑADIR SERIE
                </button>
            </div>
...
export default function App() {
    ...
    const [rutinas, setRutinas] = useState(() => {
        ...
        return saved ? JSON.parse(saved) : { 'Pecho - Triceps': [] };
    });
    ...
    // OPTIMIZACIÓN AQUÍ: He quitado el delay de 250ms que causaba el lag
    ...
    const actualizarSerie = (idEjercicio, idSerie, campo, valor) => {
        setRutinas(prev => {
            ...
            if (campo === 'eliminar') {
                ...
                copiaEj.series = copiaEj.series.map(s => s.id === idSerie ? { ...s, [campo]: valor } : s);
            }
            ...
            for (const grupo in actualizado) {
                actualizado[grupo] = actualizado[grupo].map(ejercicio => ({
                    ...
                    series: ejercicio.series.map(serie => ({
                        ...serie,
                        peso: serie.nuevoPeso && serie.nuevoPeso !== '' ? serie.nuevoPeso : (serie.peso ?? ''),
                        reps: serie.nuevoReps && serie.nuevoReps !== '' ? serie.nuevoReps : (serie.reps ?? ''),
                        nuevoPeso: '', nuevoReps: ''
                    }))
                }));
            }
    ...
    return (
        <div className="min-h-screen bg-[#05070a] text-gray-200 font-sans pb-20 overflow-x-hidden">
            <GlobalStyles />
            <main className="p-4 max-w-xl mx-auto">
                {gruposel === null ? (
                    <div className="pt-8 animate-in fade-in duration-300">
                        ...
                        <div className="space-y-3">
                            {Object.keys(rutinas).map(grupo => (
                                <div key={grupo} className="flex items-stretch rounded-2xl overflow-hidden shadow-lg">
                                    <button onClick={() => setgruposel(grupo)} className="flex-1 bg-[#111318] border-y border-l border-gray-800 p-6 text-left active:bg-[#1c212c]">
                                        <span className="text-xl font-black text-gray-100 uppercase tracking-tight">{grupo}</span>
                                    </button>
                                    {/* BOTÓN RESTAURADO SIN BORDES EXTRAÑOS */}
                                    <button onClick={() => { if(window.confirm(`¿Borrar "${grupo}"?`)) { const c = {...rutinas}; delete c[grupo]; setRutinas(c); }}} className="bg-[#111318] border border-gray-800 w-16 flex items-center justify-center text-gray-600 hover:text-red-500 border-l-0">
                    ...
                    <div className="animate-in slide-in-from-right-4 duration-300">
                        <div className="sticky top-0 bg-[#05070a]/95 backdrop-blur-md z-30 py-4 border-b border-gray-800 mb-6 flex items-center gap-4">
                            <button onClick={() => setgruposel(null)} className="p-2 bg-gray-800/50 rounded-lg text-gray-300 active:scale-95"><svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg></button>
                ...
                {confirmacionVisible && (
                    <div className="fixed top-10 left-1/2 -translate-x-1/2 bg-green-500 text-black px-8 py-3 rounded-full font-bold shadow-2xl z-50 animate-bounce">Guardado ✅</div>
                )}
            </main>
        </div>
    );
